...
Evolve Nagare from regex-based file updates to a template-based file generation system using Vento templates (`.vto` files), similar to how Lume SSG processes templates with data merging.
...
Implement a template-based system where files are generated from Vento templates (`.vto` files) with merged release data.
...
- {{ feature.name }}{{ if feature.description }}: {{ feature.description }}{{ /if }}
{{- /for }}
...
---
*Generated by [Nagare](https://github.com/RickCogley/nagare) on {{ buildDate | formatDate }}*
```
...
This template-based approach would make Nagare more powerful and reliable, following established patterns from successful tools like Lume. The use of `.vto` extension ensures good tooling support and clear separation between templates and generated files.

...
Deno.test("template security - strict mode blocks dangerous patterns", async () => {
  ...
  // Process execution
  await assertRejects(
    () => processor.processTemplate('{{ Deno.Command("rm", { args: ["-rf", "/"] }) }}', mockTemplateData),
    Error,
    "validation failed"
  );
  ...
  // Constructor escape
  await assertRejects(
    () => processor.processTemplate('{{ "".constructor.constructor("return process")() }}', mockTemplateData),
    Error,
    "validation failed"
  );
...
Deno.test("template security - safe templates work in all modes", async () => {
  for (const mode of ["strict", "moderate", "disabled"] as const) {
    ...
    const result2 = await processor.processTemplate(
      ...
      { ...mockTemplateData, versionComponents: { major: 1, minor: 2, patch: 3, prerelease: null } }
    );

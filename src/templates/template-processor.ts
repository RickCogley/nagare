 ...
 * @description Template processing and file generation for version files.
 *
 * Uses the Vento template engine for robust template processing with custom filters
 * and security sandboxing. Supports built-in templates (TypeScript, JSON, YAML) and
 * custom user templates with comprehensive security validation.
 *
 ...
 * ```typescript
 * const processor = new TemplateProcessor(config);
 * const versionFile = await processor.generateVersionFile({
 *   version: "1.2.3",
 *   buildDate: new Date().toISOString(),
 *   gitCommit: "abc123",
 *   // ... other data
 * });
 * await Deno.writeTextFile("./version.ts", versionFile);
 * ```
 *
 * @example Custom filters in templates
 * ```typescript
 * const template = `
 *   Version: {{ version }}
 *   Commit: {{ gitCommit | shortHash }}
 *   Data: {{ metadata | jsonStringify(2) }}
 * `;
 * const result = await processor.processTemplate(template, data);
 * ```
 *
...
import { createSecurityLog, sanitizeErrorMessage } from "../validation/security-utils.ts";
 ...
 * @since 0.9.0
 *
 * @example Creating a processor with custom configuration
 * ```typescript
 * const config = {
 *   versionFile: {
...
export class TemplateProcessor {
   ...
   *
   * @param {NagareConfig} config - Nagare configuration object
   *
   * @example
   * ```typescript
   * const processor = new TemplateProcessor({
   *   versionFile: { template: TemplateFormat.JSON },
  ...
  async processTemplate(template: string, data: TemplateData): Promise<string> {
    ...
    const validation = this.validateTemplateSecure(template);
    if (!validation.valid) {
    ...
    // Prepare enhanced template data outside try block so it's available in catch
